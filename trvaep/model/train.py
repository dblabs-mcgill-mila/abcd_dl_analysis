...
class Trainer:
    ...
    def make_dataset_abcd(self):
        ...
        if self.model.pca_input is True:
            if self.model.separate_pca is True:
                return data_set_train, data_set_valid, orig_train_data, orig_validate_data, pca_model_num, pca_model_cat
            else:
                return data_set_train, data_set_valid, orig_train_data, orig_validate_data, pca_model
    ...
    def train(self, n_epochs=100, batch_size=256, early_patience=15):
        ...
        if self.model.pca_input is True:
            if self.model.separate_pca is True:
                dataset_train, dataset_valid, _, _, _, _ = self.make_dataset_abcd()
            else:
                dataset_train, dataset_valid, _, _, _ = self.make_dataset_abcd()
        else:
            dataset_train, dataset_valid = self.make_dataset_abcd()
        data_loader_train = torch.utils.data.DataLoader(dataset=dataset_train,
                                                        batch_size=batch_size,
