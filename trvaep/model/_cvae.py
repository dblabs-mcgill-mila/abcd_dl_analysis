...
from .helper_module import Encoder, Decoder, Decoder_split_vars, Decoder_sigmoid_op
...
class CVAE(nn.Module):
    ...
    def __init__(self, input_dim, num_classes=None, encoder_layer_sizes=[64, 32],
                 ...
                 dr_rate=0.2, use_mmd=False, beta=1, output_activation="linear", pca_input = False, separate_pca = False, pca_dim = None,
                 var_type_split=None, top_std_cols = False, splitter = 'manual', decoder_choice = 'base', \
                cols_split_idx = None):
        super().__init__()
        ...
        self.var_type_split = var_type_split
        self.top_std_cols = top_std_cols
        self.splitter = splitter
        self.decoder_choice = decoder_choice
        self.cols_split_idx = cols_split_idx
        if self.dr_rate > 0:
            self.use_dr = True
        else:
            self.use_dr = False
        ...
        elif self.decoder_choice == 'split_op':
            self.decoder = Decoder_split_vars(decoder_layer_sizes, self.latent_dim,
                                self.use_bn, self.use_dr, self.dr_rate, self.cols_split_idx, self.use_mmd, self.num_cls, self.op_activation)
        elif self.decoder_choice == 'sigmoid_op':
            self.decoder = Decoder_sigmoid_op(decoder_layer_sizes, self.latent_dim,
                                self.use_bn, self.use_dr, self.dr_rate, self.use_mmd, self.num_cls, self.op_activation)
